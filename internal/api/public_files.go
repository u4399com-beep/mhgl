...
func (d Deps) publicFeedbackPost(w http.ResponseWriter, r *http.Request) {
	...
	if ip != "" {
		since := store.NowMS() - 3600_000
		cnt, err := d.DB.Count(`SELECT count(*) FROM "Feedback" WHERE ip=? AND createdAt>=?`, ip, since)
		if err == nil && cnt >= 5 {
	...
	fbID := d.DB.NewID()
	_, err := d.DB.Exec(`INSERT INTO "Feedback" (id,type,contact,content,url,siteId,userAgent,ip,status,createdAt,updatedAt)
VALUES (?,?,?,?,?,?,?,?,'new',?,?)`,
		fbID, typ, nilIfEmpty(contact), content, nilIfEmpty(feedbackURL), nilIfEmpty(siteID),
		nilIfEmpty(userAgent), nilIfEmpty(ip), store.NowMS(), store.NowMS())
	if err != nil {
		apiErr(w, http.StatusInternalServerError, "服务器内部错误")
		return
	}
