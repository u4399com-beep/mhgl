...
// Sink 回调面抽象(R55 单体桥注入缝): 8 种 kind 的语义化方法集。
// *Client(HTTP 回调, mini-services 时代契约 §2)与 internal/crawl/bridge(单体直连 store)
// 共同实现本接口; task.Task.cb 依赖本抽象, 由 Manager 的 sink 工厂在构造任务时注入。
// 语义权威: go-callback/route.ts(桥侧逐 kind 移植)与本文件(HTTP 侧)。
type Sink interface {
	Log(ctx context.Context, level, message string) error
	Status(ctx context.Context, status, note string) error
	Stats(ctx context.Context, s StatsPayload) error
	SendProgress(ctx context.Context, payload interface{}, force bool) error
	Book(ctx context.Context, p BookPayload) (BookDecision, error)
	Chapters(ctx context.Context, p ChaptersPayload) (ChaptersDecision, error)
	Contents(ctx context.Context, p ContentsPayload) error
	Cover(ctx context.Context, p CoverPayload) error
}
