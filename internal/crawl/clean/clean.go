...
func defaultAdPatterns() []string {
	...
	out = append(out, []string{
		...
		// ---- R68-b 生产 DB 旁证实证增强(万相之王/xyetianlian 现役残留, 见 clean_test.go
		// TestR68b* 正例+防误伤反例) ----
		// 杰奇CMS 书页页脚水印行("作者：X所写的《Y》无弹窗免费全文阅读为转载作品,章节由网友发布。"):
		// 简介出口(TS 语义即只跑缺省 patterns, cleanIntro 不消费规则自定义清单)残留实证,
		// 收入缺省表使 intro/content 双出口同时覆盖; 全短语链置信度高, 无正文误伤面
		`作者：[^<>\n]{0,30}所写的《[^<>\n]{1,60}》无弹窗[^<>\n]{0,40}转载作品`,
		// 翻页标记变体("本章未完，点击[下一页]继续阅读>>" —— [13] 括号形态只锚
		// "(本章未完…请翻页)", 方括号/箭头变体漏网; 尾部 {0,16} 消费 "--&gt;&gt;"(10 码点)
		// 类箭头残尾防 <p> 只剩箭头垃圾; "本章未完"+"点击…继续阅读"组合恒为站点翻页噪声)
		`本章未完[，,]?点击[^\n<]{0,10}继续阅读[^\n<]{0,16}`,
...
// maskLead/maskTrail [R69-b] 掩码 token 两侧的「装饰边界」单类: 空白族(同 lineWs
// 逐字对齐)+括号对(（【[/）】])+破折/星点装饰(-–—·•~*＊)。源站页脚水印常见「（URL）」
// 「【URL】」「—— URL ——」整行形态 —— 修前 [2] 行/段锚只容空白与单标点, 括号内 scheme
// URL 因掩码保护对域名模式不可见而整行漏网(探针实证); 本类只在 [2] 整行/整段回收形态
// 中作用于掩码两侧, 行内仍有正文时锚不成立 → 正文括注 URL(「官网（http://x.com）公告」)
// 不受影响。实现注记: ①解释型字符串(unicode 落原字符, 26 rune/类) —— raw string 里
// \uXXXX 会成 RE2 非法转义整条模式静默失效(R62-c3 同款事故形态, TestAllAdPatterns-
// Compile 巡检兜底) ②'-' 置于类首防 RE2 字符类区间误读('\[-–' 会被读作 [ 到 – 的大
// 区间) ③[2] 拆为 [2]/[2b] 两条, 各自保持 compileAdPattern 300 rune 上限内
const maskLead = "[-\\s\u00a0\u1680\u2000-\u200a\u202f\u205f\u3000（【\\[–—·•~*＊]*"
const maskTrail = "[-\\s\u00a0\u1680\u2000-\u200a\u202f\u205f\u3000）】\\]–—·•~*＊]*"

// wsU [R69-b] 括号内衬空白类: Go \s 不含 U+00A0/U+3000 —— WAP 站页脚标记常用
// &nbsp;/全角空格内衬(「（\u00a0本章完\u00a0）」, 探针实证 [5]/[13]/[14] 全漏),
// 括号标记族的 \s 统一本类。解释型字符串(同 lineWs 口径)
const wsU = "[\\s\u00a0\u2000-\u200a\u202f\u205f\u3000]*"
...
var coreAdPatterns = []string{
	...
	// [R69-b] 修: 掩码两侧改 maskLead/maskTrail 装饰边界(括号对/破折装饰),
	// 「（http://x.com）」「【http://x.com】」「—— URL ——」整行水印漏网修复;
	// 原 [2] 三臂拆为 [2](行/br 臂)+[2b](<p> 臂) 两条 —— 各自保持 compileAdPattern
	// 300 rune 上限内(合并式已越限被静默跳过, 整行 URL 回收整体失效, 探针+测试实证)
	"(?m)^" + maskLead + "\uE000\\d+\uE001" + maskTrail + "[。．.!！]?" + maskTrail + "$" +
		"|<br\\b[^>]*>" + maskLead + "\uE000\\d+\uE001" + maskTrail + "(?:<br\\b[^>]*>)?",
	// [2b] 整行仅 URL —— <p> 包裹形态(R69-b 拆分自 [2], 语义同上)
	"<p[^>]*>" + maskLead + "(?:<a\\b[^>]*>)?" + maskLead + "\uE000\\d+\uE001" + maskTrail + "(?:</a>)?" + maskTrail + "</p>",
