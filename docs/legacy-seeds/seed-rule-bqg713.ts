  ...
  config: {
    ...
    toc: {
      ...
      fields: {
        ...
        // [R62-c] 对齐 R61 deqixs 真虫2 修法: toc 合成「真实章节 URL」(裸 apige.cc 形态),
        // 代理包裹交回引擎 contentProxyUrl(matchesTemplateOrigin 同源自指包裹会被跳过 →
        // loopback 直连被 SSRF 守卫硬拒, 旧 degrade-native 契约在 Go 引擎不成立)。
        url: { type: 'const', expression: 'https://apige.cc/api/chapter?id={q.id}&chapterid={index}' },
      },
    ...
    content: {
      ...
      fields: {
        ...
        // [R62-c] 正文改 css body(对齐 R61 deqixs 修法): FetchContentRef 经 /unlock 解包后
        // {ok,content} JSON 已被引擎转成 <p> HTML, content 段看到的是段落 HTML 非 JSON。
