...
- `scripts/` 根目录只保留「活资产」：站点 mock（`mock-novel-site.ts`）、限流测试服务（`ratelimit-site.ts`）。种子脚本 R62-a 起迁 `docs/legacy-seeds/`；运维工具 `export-autofill-rules.ts` 与三套 Docker 断言及 Docker 链本体已随 **R66-d Docker 链退役**移入 `docs/archive/docker/`（`gen-builtin-rules.ts` 在 legacy-seeds；`backfill-book-num.ts` 为 Prisma 时代一次性回填，R64-d 删）。
