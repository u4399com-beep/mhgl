// cc-b 端到端验证: shudugu.org(速读谷) 单本 3 章小任务(3 线程, 间隔 300~600ms, http 引擎直连)
// → 正文质量检查(站名/域名/推广/\u0000 残留) → 清理还原 DB
// 用法: bun run scripts/e2e-cc-b.ts
// 流程: 按规则名找 ruleId → 建任务 → start → 轮询(已采章节≥3 即 stop) → 等停 →
//       质量检查(逐章全文 junk 扫描) → 删任务+删书(章节/tags 级联) → DB 残余核对
const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms))

const BASE = 'http://localhost:3000'
const RULE_NAME = '速读谷 (shudugu.org)'
const BOOK_URL = 'https://www.shudugu.org/51/'
const BOOK_NAME = '捞尸人'
const TASK_NAME = 'cc-b-e2e-shudugu'
...
async function main() {
  ...
  // 3) 轮询: 已采(fetched=true)章节 ≥3 即停(http 引擎每章 1~2s, 预算 150s)
  ...
  if (fetchedCount < 3) console.log(`!! 150s 内未采满 3 章(实际 ${fetchedCount}), 继续停止流程检查现场`)
  ...
  if (bookId) {
    ...
    for (const c of items.slice(0, 5)) {
      ...
      const junk = [
        'shudugu', 'sudugu', '速读谷', '谷内无错', '请记住本站', '纯手打',
        'window.location', 'http://', '下一页', '上一页', '下一章',
      ...
      console.log(
        ...
        `junk=${JSON.stringify(junk)} null=${nullChars} 开头=${JSON.stringify(head)}`,
      )
    }
    console.log(qualityOk ? '✅ 正文质量检查通过(无站名/域名/导航残留/\\u0000)' : '❌ 正文质量检查未过')
  ...
  const residualTask = taskArr.filter((t: any) => t.id === taskId || t.name === TASK_NAME)
