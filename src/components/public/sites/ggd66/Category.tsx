// ============================================================
// [R27-6-2] ggd66(格格党) 分类/书库页克隆 —— 按 https://www.ggd66.com/sort/ 真站快照逐节还原
// (/tmp/r27-f/ggd66-sort.html + ggd66-style.css 实测)
//
...
export function Ggd66Category({ data, loading, error, catName, cat, page }: SiteCategoryProps) {
  ...
  // [R27-6-2] .class 分类导航条(fetchCategories; 真站 8 分类 11.111% 等分)
  ...
  return (
    <div className="mx-auto w-[90%] max-w-[1200px] pb-10" style={{ color: TEXT_BODY }}>
          ...
          <ul className="flex flex-wrap">
            <li className="w-1/2 py-2.5 text-center text-[16px] sm:w-1/4 lg:w-[11.111%]">
              <button
                ...
                style={{ color: !cat ? '#f50' : GREEN_LINK, fontWeight: !cat ? 700 : 400 }}
                aria-label="浏览全部分类"
              >
                全部
              </button>
            ...
            {cats.map((c) => (
              <li key={c.id} className="w-1/2 py-2.5 text-center text-[16px] sm:w-1/4 lg:w-[11.111%]">
                <button
                  ...
                  className="transition-colors hover:text-[#f50]"
                  style={{ color: cat === c.id ? '#f50' : GREEN_LINK, fontWeight: cat === c.id ? 700 : 400 }}
      ...
      {/* ============ .content.book 列表白盒 ============ */}
      <div className="ggd-book mt-2.5 border bg-white px-2.5 pb-2.5 shadow-[0_1px_1px_rgba(0,0,0,0.05)]" style={{ borderColor: LINE }}>
          ...
          <>
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {books.map((b, i) => (
                ...
                <div key={b.id} className="ggd-bookbox relative">
                  <div className="ggd-p10 h-full overflow-hidden rounded-[4px] border border-dashed p-2.5 pl-9" style={{ borderColor: LINE }}>
                    <span
                      ...
                      style={{ background: TEAL, color: '#eee' }}
                      aria-hidden
                    >
                      {(page - 1) * size + i + 1}
                    </span>
                    <div className="pl-0.5">
                      ...
                      <div className="truncate text-[14px]">作者：{b.author}</div>
                      {/* 真站第三行「阅读量：N」无数据契约 → 分类名替代(声明) */}
                      <div className="truncate text-[14px]">分类：{b.category || '—'}</div>
                      <div className="truncate text-[14px]">字数：{formatWords(b.wordCount)}</div>
            ...
            {/* .pages 分页(真站 2494 页窗口化 ±4) */}
            <Pages page={page} totalPages={totalPages} onGo={(p) => navigate({ view: 'category', cat, page: p })} />
          </>
        ) : (
