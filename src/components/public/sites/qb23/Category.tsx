// ============================================================
// [R28-2c] qb23 铅笔小说(www.23qb.net) 分类页克隆 —— 基础五视图之 Category
// 真站快照(R28 实测): /tmp/r28-2c/qb23/qb23-category.html(/book/lastupdate_0_6_0_0_0_0_0_1_0.html 直抓)
// 真站 DOM: main#main.wrapper > .content
//   ├ .page-heading > .box(白卡 padding 25px / radius 18px / 大投影)
//   │   ├ .library-box×5 筛选行(scroll-content flex: b.library-item.library-item-first 标签 + .library-list 筛选片)
//   │   │   行1 分类(全部/言情小说/都市小说/…14 类) 行2 首字(全部/[A]…[Z]) 行3 字数(30万以下…400万以上)
//   │   │   行4 排序(周点击/月点击/周推荐/月推荐/新书榜/字数/收藏数/更新时间/入库时间) 行5 进度(全部/新书连载/已经完本)
//   │   │   chip: padding 0 20px / line-height 35px / 14px(媒体查询 12px) / radius 10px;
//   │   │   .library-item.selected #fef0e5 底 + #ff2a14 字 + 700(未选中 #f3f5f7 底 hover #eaedf1)
//   │   └ h1.library-stat「玄幻魔法全部_更新时间_全部」(14px→12px, rgba(0,0,0,.62))
//   ├ .module > .module-list > .module-items > .module-item×30 封面网格(同首页卡型, 无序号角标;
//   │   真站分类页 caption span 为空 → 克隆省略 caption 渐变条)
//   └ .module-footer > #page 分页: span 第X/Y页 + a.page-previous + strong 当前页(#ff2a14 白字 50px 圆)
//       + a 页码(#f3f5f7 底 hover #eaedf1, radius 50px, min-width 40px) + a.page-next
// 筛选降级: 首字/字数/排序/进度四行真站可点但数据层无对应查询参数 → 视觉复刻为 aria-disabled 装饰片,
//   仅「分类」行可交互(navigate category)。library-stat 追加「共 N 本」满足页头口径。
// QbFilterRow/QbFilterChip/QbPageBtn/pageWindowOf 导出供 Fulltext.tsx 复用(真站完本页同款模板)。
// ============================================================
'use client'
...
/** [R28-2c-7] .library-item 筛选片 — active=selected(暖杏底/红字/700; 色彩走 class 保 hover 可覆盖) */
export function QbFilterChip({
  label,
  active,
  onClick,
  title,
...
/** [R28-2c-8] .library-box 筛选行 — 行首 <b> 标签 + .library-list 横滚片组 */
export function QbFilterRow({ label, children }: { label: string; children: ReactNode }) {
  return (
...
/** [R28-2c-9] #page 页码钮(#f3f5f7 底 hover #eaedf1, radius 50px min-width 40px) */
export function QbPageBtn({
  children,
  onClick,
  disabled,
  ariaLabel,
...
/** 页码窗口: 当前页 ±3, 首末页恒显(真站罗列 1..10, 克隆窗口化避免长尾) */
export function pageWindowOf(page: number, totalPages: number): number[] {
    ...
    <div className="w-full pb-14" style={{ color: QB_TEXT }}>
      <div className="mx-auto w-full max-w-6xl px-4 pt-4 sm:px-6">
        {error ? (
          ...
          <>
            {/* ============ .page-heading > .box: 筛选行(分类/首字/字数/排序/进度) + library-stat ============ */}
            <div className="rounded-[18px] bg-white p-4 shadow-[0_7px_21px_rgba(149,157,165,0.22)] sm:p-[25px]">
              <QbFilterRow label="分类">
                <QbFilterChip label="全部" active={!cat} onClick={() => navigate({ view: 'category', page: 1 })} />
                {cats.map((c) => (
                  <QbFilterChip
                    key={c.id}
                    label={c.name}
