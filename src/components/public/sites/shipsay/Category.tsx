// ============================================================
// [R27-6b-8] shipsay(船说 CMS demo) 分类/书库页克隆 —— 船说 V4.2 store 页还原
// 素材等级: Wayback 实测 —— /tmp/r27-f2/ss-cat.html(2024 快照 demo.shipsay.com/xs/xuanhuan/1/
// 完整 DOM): .store > .store_left >
//   i#store_menu(fa-bars 筛选菜单) + .side_commend > .title(分类名) + #after_menu >
//   div(只看全本 checkbox → /quanben/xs/{slug}/{page}/) + div(全部分类 + 分类 a.onselect) +
//   ul.flex > li(.img_span > a > img.lazy + span「分类 / 状态」 + .w100 > a h2 书名 +
//   p.indent 简介 + .li_bottom > i.fa-user-circle-o 作者 + em.orange 字数 + em.blue 日期)
// 契约映射: ①「只看全本」无 status 过滤 API → 渲染禁用态(声明)
//           ②真站 8 分类 slug(xuanhuan/wuxia/dushi/lishi/kehuan/youxi/nvsheng/qita) → 库内动态分类
//           ③分页钮为家族标准形态(快照未含分页段, 家族标准声明)
// 色值: 同首页实测(#f4f4f4/#fff/#666/#1a1a1a/#ed4259/#555/#4284ed/#f0643a)。
// ============================================================
'use client'
    ...
    <div className="ss-home w-full pb-6" style={{ background: '#f4f4f4', color: C.text, fontSize: 14 }}>
      <div className="mx-auto w-full max-w-[960px] px-2 pt-3">
        {error ? (
          ...
          <>
            {/* ============ .store > .side_commend(筛选侧栏) ============ */}
            <div className="ss-card mb-3 p-3">
              <div className="ss-store">
                {/* .side_commend > .title 分类名 */}
                <h1 className="m-0 pb-1.5 text-[18px] font-bold" style={{ color: C.title }}>
                  {catName}
                </h1>
                {/* #after_menu: div(只看全本, 无 status 过滤 API → 禁用态) + div(分类链) */}
                <div className="ss-after_menu border-b pb-2" style={{ borderColor: C.line }}>
                  <label className="mr-3 inline-flex cursor-not-allowed items-center gap-1 text-[13px] opacity-60" title="全本筛选暂未开放">
                    <input type="checkbox" disabled aria-label="只看全本" />
                    只看全本
                  </label>
                  <button
                    type="button"
                    onClick={() => navigate({ view: 'category', page: 1 })}
                    className={`mr-3 text-[13px] ${!cat ? 'font-bold' : ''}`}
                  ...
                  {cats === null ? (
                    ...
                    cats.map((c) => (
                      <button
                        ...
                        className={`ss-cat mr-3 text-[13px] ${cat === c.id ? 'font-bold' : ''}`}
                ...
                {/* ul.flex 书卡列表(img_span 封面 + w100 信息) */}
                {loading && !books.length ? (
                  <ul className="ss-flex m-0 grid list-none grid-cols-1 gap-3 pt-3 sm:grid-cols-2" role="status" aria-label="书库加载中">
                        ...
                        <div className="ss-img_span relative mr-2.5 shrink-0 overflow-hidden" style={{ width: 80, height: 106 }}>
                          <button type="button" onClick={() => navigate({ view: 'book', bookId: b.id })} className="block h-full w-full cursor-pointer" aria-label={`查看《${b.name}》详情`}>
                            ...
                            >
                              ...
                              <span className="shrink-0">{b.status === 'completed' ? '全本' : '连载'}</span>
                            </span>
                          </button>
                        </div>
