// ============================================================
// [R26-2-1] 霹雳书屋 克隆首页 —— https://www.pilishuwu.com/ (wmcms 模板)
//
// 真站勘察样本: /tmp/r26/probe-www.pilishuwu.com.html(本轮 stealthy 桥直抓 2026 快照,
// 与上轮 /tmp/sites/pilishuwu-home.html 一致) + /tmp/sites/pilishuwu-style0.css(wmcms.global.css)
// + style4.css(wmcms.index.css) 实测。
//
// 板块顺序(真站 DOM 自上而下, 本组件 = mod-top-wr 头部之后的全部):
//   1. .mod-tags-wr            独家推荐米色词条条(#faead0 底 + #eed3a4 边线 + 橙块徽标,
//                              徽标渐变 #ff9126→#fd8929 + 内高光/inset 1px #ec7d4d; 书名词 hover 弹封面卡 mod-ani-info)
//   2. .in-banner-con          大封面横幅(桌面 356px 高, 底部 #f1823a 30px 书名条, hover 黑纱 50% 出信息)
//      .in-rank-wr             右侧深色热点榜(#373533 sprite 深盒, #ff9a6a 栏头, 橙/灰序号徽章,
//                              in-rank-spacing 虚线分隔, in-rank-recommend 黄字 #ffca68 推荐行)
//   3. .in-strong-wr           精品推荐(2px #ff9a6a 上边线 + in-title-big 30px 压线「精品<em>推荐</em>」;
//                              左大封 214×284 + 右 in-sign-work 作品行 18px 书名/作者/分类/字数/简介
//                              + 底部 in-sign-list 7 封面条 100×133 hover 黑纱)
//   4. .in-vip-wr              双列 580px: 左 .in-rise-wr「最新入库」(in-rise-item 封面 100×131
//                              mod-cover-list-thumb-small 98×131 #bebebe 边, hover 弹简介黑纱,
//                              mod-cover-list-name 14px #333 / intro #999 / mod-tag-item #999);
//                              右 .in-monrank-wr「月票排行」(in-monrank-tit #ff9a6a 25px 白字栏头,
//                              序号徽章 hot #908aa2 白字 / no #f0efee #333, 票数 em #ff8a2b)
//   5. .in-main-wr+.in-side-wr 主列双分类封面列(mod-cover-list 135×177 封面 + 底部 22px
//                              rgba(0,0,0,.6) 章节黑条 mod-cover-list-mask + 书名/简介/标签)
//                              + 侧栏 .in-phlist-wrap 点击排行(#ff9a6a 栏头 48px 白字 + #f7f7f7
//                              列头 40px) + .in-rise-ta 最近更新表(斑马纹 #fafafa/行线 #ededed)
//
// 实测色板(pili 基因, 主控 themes.ts 同源): 主橙 #fd8929 / 浅橙 #ff9a6a / 按钮橙 #f89157
// (ui-btn-orange, hover #f59966, active #f1854b, 边 #ec7d4d) / 米色 #faead0 + #eed3a4 边 /
// 红强调 #d71704(最新章红 #cd1604) / 深榜 #373533 / 文字 #333·#555·#666·#999 / 版心 ui-wm 1200px。
//
// 数据: props.books = 最新 48 本; 板块切分自排: banner+强档 0..10, 最新入库 11..30(20 本),
// 分类封面列按 category 分组取前 2 组, 最近更新表 slice(0,14); 点击/月票榜维度另拉
// fetchBooks(sort:'words')(失败回退 props.books 按字数排序)。空数组全部安全降级。
// ============================================================
'use client'
...
/** [R26-2-3] 独家推荐词条 + hover 封面弹出卡(真站 mod-tags-wr / mod-ani-info) */
...
/** [R26-2-4] 横幅行 = 大封面 banner + 深色热点榜(真站 in-banner-con + in-rank-wr) */
function PiliBanner({ featured, rankBooks, loading }: { featured?: BookItem; rankBooks: BookItem[]; loading: boolean }) {
  ...
  return (
    <div className="mx-auto grid w-full max-w-6xl gap-5 px-4 pt-5 sm:px-6 lg:grid-cols-[1fr_257px]">
      {/* banner 主图(真站 356px 高, 底部 #f1823a 书名条, hover 黑纱 50% 出信息) */}
      {featured ? (
        <div
...
/** [R26-2-5] 板块标题(真站 in-title-wr: 2px #ff9a6a 上边线 + in-title-big 30px 压线;
 *  真站标题均为「xx<em>xx</em>」双色形态: 精品<em>推荐</em>/最新<em>入库</em>/男频<em>小说</em>, em 字 #ff9a6a 24px) */
function PiliSectionTitle({ plain, em, more }: { plain: string; em?: string; more?: { label: string; go: () => void } }) {
  const { navigate } = usePublic()
  return (
    <div className="border-t-2" style={{ borderColor: PILI_ORANGE_LIGHT }}>
      <div className="flex items-end justify-between">
        <h2 className="inline-block -translate-y-2 bg-[#fafafa] pr-6 text-2xl font-normal leading-[30px] sm:text-[30px]" style={{ color: PILI_TITLE }}>
          {plain}
          {em ? <em className="not-italic" style={{ color: PILI_ORANGE_LIGHT, fontSize: 24 }}>{em}</em> : null}
        </h2>
        {more ? (
          <button
            type="button"
...
/** [R26-2-6] 强档推荐 = 左大封 214×284 + 右作品行×3 + 底部封面横条(真站 in-sign-* / in-sign-list) */
function PiliStrong({ books, loading }: { books: BookItem[]; loading: boolean }) {
  ...
  return (
    <section className="mx-auto mt-8 w-full max-w-6xl px-4 sm:px-6">
      <PiliSectionTitle plain="精品" em="推荐" />
      <div className="mt-4 grid gap-5 lg:grid-cols-[214px_1fr]">
        {/* 左侧大封(in-sign-cover 214×284, #eeeded 边) */}
        {featured ? (
          <div
            {...bookNavProps(navigate, featured.id)}
            aria-label={`查看《${featured.name}》详情`}
        ...
        <div className="min-w-0">
          {/* 右侧作品行(in-sign-work: 书名 18px/30px + 作者/分类/字数 + 简介) */}
...
/**
 * [R26-2-7] VIP 区双列(真站 in-vip-wr 580px×2):
 * 左 in-rise-wr「最新入库」: 封面 100×131(mod-cover-list-thumb-small 98×131 #bebebe 边)
 *   hover 弹简介黑纱, mod-cover-list-name 14px #333(hover #ff9a6a) / intro #999 22px / tag #999;
 * 右 in-monrank-wr「月票排行」: in-monrank-tit #ff9a6a 25px 白字圆角栏头 + 行 26px
 *   序号徽章(hot #908aa2 白字/no #f0efee #333) + 票数 em #ff8a2b。
 */
function PiliVipSection({ latest, rank, loading }: { latest: BookItem[]; rank: BookItem[]; loading: boolean }) {
  const { navigate } = usePublic()
  if (loading) {
    return (
      <div className="mx-auto mt-8 grid w-full max-w-6xl gap-8 px-4 sm:px-6 lg:grid-cols-2" aria-hidden>
        {[0, 1].map((c) => (
          <div key={c}>
            <Sk className="h-8 w-40" />
            <div className="mt-4 flex gap-[21px] overflow-hidden">
              {Array.from({ length: 5 }).map((_, j) => (
                <div key={j} className="w-[86px] shrink-0 space-y-2 sm:w-[100px]">
                  <Sk className="h-[131px] w-full rounded-[2px]" />
  ...
  if (!latest.length && !rank.length) return null
  return (
    <section className="mx-auto mt-8 grid w-full max-w-6xl gap-8 px-4 sm:px-6 lg:grid-cols-2">
      {/* 最新入库(in-rise-wr / in-rise-item) */}
      <div aria-label="最新入库">
        <div className="flex items-center justify-between border-b border-[#f0e6d2] pb-2">
          <h3 className="flex items-center gap-1.5 text-xl font-normal" style={{ color: PILI_TITLE }}>
            ...
            最新<em className="not-italic" style={{ color: PILI_ORANGE_LIGHT }}>入库</em>
          </h3>
          <button
            type="button"
            onClick={() => navigate({ view: 'category' })}
        ...
        <div className="mt-3 flex gap-[21px] overflow-x-auto pb-1">
          {latest.slice(0, 10).map((b) => (
            <div key={b.id} className="group w-[86px] shrink-0 cursor-pointer sm:w-[100px]" {...bookNavProps(navigate, b.id)} aria-label={`查看《${b.name}》详情`}>
              {/* 封面 + hover 简介黑纱(真站 .content 悬浮层) */}
              <div className="relative overflow-hidden rounded-[2px] border border-[#bebebe]">
                <BookCover name={b.name} cover={b.cover} showAuthor={b.author} style={{ borderRadius: 0 }} className="h-[114px] w-full sm:h-[131px]" />
                <span aria-hidden className="absolute inset-0 hidden bg-black/80 p-1.5 text-[10px] leading-[15px] text-white/90 opacity-0 transition-opacity duration-200 group-hover:opacity-100 sm:line-clamp-6 sm:block">
                  {b.intro || '暂无简介'}
                </span>
              </div>
              <p className="mt-[7px] truncate text-sm text-[#333333] transition-colors group-hover:text-[#ff9a6a]">{b.name}</p>
              <p className="h-[22px] truncate text-xs leading-[22px] text-[#999999]">{b.author}</p>
              <p className="h-5 truncate text-xs text-[#999999]">{b.category}</p>
            </div>
          ))}
        </div>
      </div>

      {/* 月票排行(in-monrank-wr / in-monrank-tit / in-monrank-list) */}
      <div aria-label="月票排行">
        <div className="flex items-center justify-between border-b border-[#f0e6d2] pb-2">
          <h3 className="flex items-center gap-1.5 text-xl font-normal" style={{ color: PILI_TITLE }}>
            <TrendingUp className="h-4 w-4" style={{ color: PILI_ORANGE }} aria-hidden />
            月票<em className="not-italic" style={{ color: PILI_ORANGE_LIGHT }}>排行</em>
          </h3>
          <span className="text-xs text-[#999999]">按字数热度排序</span>
        </div>
        <div className="mt-3 border" style={{ borderColor: '#ebeae8' }}>
          <div className="flex h-[25px] items-center justify-between px-3 text-[13px] text-white" style={{ background: PILI_ORANGE_LIGHT, borderRadius: '2px 2px 0 0' }}>
            <span>名次</span>
            <span>作品 / 字数</span>
          </div>
          <ul style={{ listStyle: 'none' }}>
            {rank.slice(0, 10).map((b, i) => (
              <li key={b.id} className="flex items-center gap-2 border-b border-[#f2f2f2] px-3 py-[5px] last:border-b-0">
                <span
                  className="inline-block h-[16px] w-[17px] shrink-0 text-center text-[11px] leading-[16px]"
                  style={i < 3 ? { color: '#ffffff', background: '#908aa2' } : { color: '#333333', background: '#f0efee' }}
                >
                  {i + 1}
                </span>
                <button
                  ...
                  onClick={() => navigate({ view: 'book', bookId: b.id })}
                  aria-label={`查看《${b.name}》详情`}
                  className="min-w-0 flex-1 truncate text-left text-[13px] transition-colors hover:text-[#ef7661]"
                  style={{ color: PILI_TEXT }}
                >
                  {b.name}
                </button>
                <span className="shrink-0 text-xs tabular-nums" style={{ color: '#ff8a2b' }}>
...
/** [R26-2-8] 分类封面列(mod-cover-list: 135×177 封面 + 22px rgba(0,0,0,.6) 章节黑条 + 书名/简介/标签) */
function PiliCoverColumn({ title, books }: { title: string; books: BookItem[] }) {
  const { navigate } = usePublic()
  if (!books.length) return null
  // 真站栏目标题双色: 男频<em>小说</em>/女频<em>小说</em>/纯爱<em>小说</em> → 尾缀「小说」入 em, 其余取末两字
  const emPart = title.endsWith('小说') ? '小说' : title.slice(-2)
  const plainPart = title.slice(0, title.length - emPart.length)
  return (
    <section aria-label={`${title}推荐`}>
      <div className="mb-3 flex items-center justify-between border-b border-[#f0e6d2] pb-2">
        <h3 className="flex items-center gap-1.5 text-xl font-normal" style={{ color: PILI_TITLE }}>
          <Flame className="h-4 w-4" style={{ color: PILI_ORANGE }} aria-hidden />
          {plainPart}
          <em className="not-italic" style={{ color: PILI_ORANGE_LIGHT }}>{emPart}</em>
        </h3>
        <button
          type="button"
...
/** [R26-2-9] 侧栏: 点击排行(in-phlist-wrap) + 最近更新表(in-rise-ta 斑马纹) */
function PiliSide({ hot, latest, loading }: { hot: BookItem[]; latest: BookItem[]; loading: boolean }) {
  ...
  return (
    <aside className="min-w-0">
      ...
      {/* 最近更新表(in-rise-ta: 分类 chip + 书名/最新章 + 时间, 斑马纹 #fafafa 行线 #ededed) */}
...
export function PiliHome({ books, loading }: SiteHomeProps) {
  ...
  // [R26-2-10] 第二维度: 字数最多(点击/月票榜基因)喂 热点榜/月票排行/点击排行;
  // 失败回退 props.books 按字数排序。
  ...
  // 分类封面列: 48 本按分类分组, 取书目最多的两组(对应真站 in-main-wr 男频/女频双列)
  ...
  return (
    <div className="w-full pb-12" style={{ color: PILI_TEXT }}>
      ...
      {/* ============ 强档推荐区(真站 in-strong-wr, 标题「精品推荐」) ============ */}
      <PiliStrong books={books} loading={loading} />

      {/* ============ VIP 区: 最新入库 + 月票排行(in-vip-wr 双列 580px) ============ */}
      <PiliVipSection latest={books.slice(11, 31)} rank={hotList} loading={loading} />
