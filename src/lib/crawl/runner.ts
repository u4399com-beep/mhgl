        ...
        for (let p = cfg.task.listStart; p <= cfg.task.listEnd; p++) {
          ...
          try {
            ...
            if (e?.name === 'GlobalSemTimeout' || e?.name === 'MemoryHaltError') {
              ...
              // [R49-10] 计数冻结(恢复): 暂停已触发(rt.paused)时冻结熔断计数 —— 同一熔断窗口内
              //  后续页再命中时不再重复计数/刷日志(修前实测溢出形态 "4/3"); 循环头会停在
              //  rt.paused 等待门, 恢复后由 [R51-3-b] wasPaused 门归零重新累计。此处静默跳过
          ...
          while (true) {
            ...
            try {
              ...
              if (isStale()) {
                ...
                // [R49-10] 计数冻结(恢复): 同批其他在途 worker 也会吃 MemoryHaltError, 暂停已触发后
                //  (rt.paused)不再重复计数/刷日志/落进度(修前实测溢出形态 "4/3"); GlobalSemTimeout
                //  亦不喂本臂([R51-3-b] 错误归因修复), 走下方 else 仅告警
                if (e?.name === 'MemoryHaltError' && !rt.paused) {
        ...
        batch.map(async (item) => {
          ...
          try {
            ...
            if (e?.isFetchTimeout) {
              ...
              // [R49-10] 计数冻结(恢复): 同批其他在途 worker 也会吃 MemoryHaltError, 暂停已触发后
              //  (rt.paused)不再重复计数/刷日志(修前实测溢出形态 "4/3" 即此竞态); GlobalSemTimeout
              //  亦不喂本臂([R51-3-b] 错误归因修复口径), 走下方 else 仅告警
